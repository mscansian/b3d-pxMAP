...
Function PXWC_LoadMap(Map_Filename$,Only_File=0)
	...
	For t.triangle = Each triangle
		If t\on_File = 1
			...
			Img = LoadImage(GetProperty(0,"texture")+s\texture+".bmp")
			IW# = Float(ImageWidth(Img))
			IH# = Float(ImageHeight(Img))
	
			t\u1 = ((t\x1 * t\tv1x + t\y1 * t\tv1z + t\z1 * t\tv1y)/s\scalex + s\shiftx)/IW
			t\u2 = ((t\x2 * t\tv1x + t\y2 * t\tv1z + t\z2 * t\tv1y)/s\scalex + s\shiftx)/IW
			t\u3 = ((t\x3 * t\tv1x + t\y3 * t\tv1z + t\z3 * t\tv1y)/s\scalex + s\shiftx)/IW

			t\v1 = ((t\x1 * t\tv2x + t\y1 * t\tv2z + t\z1 * t\tv2y)/s\scaley + s\shifty)/IH
			t\v2 = ((t\x2 * t\tv2x + t\y2 * t\tv2z + t\z2 * t\tv2y)/s\scaley + s\shifty)/IH
			t\v3 = ((t\x3 * t\tv2x + t\y3 * t\tv2z + t\z3 * t\tv2y)/s\scaley + s\shifty)/IH
		EndIf
	Next
		...
		If t\On_File = 1 And Only_File = 0
			...
			If Found = 1
				;Cria Triangulo
					...
					;Texture
					t2\u1 = ((t2\x1 * t2\tv1x + t2\y1 * t2\tv1z + t2\z1 * t2\tv1y)/s\scalex + s\shiftx)/IW
					t2\u2 = ((t2\x2 * t2\tv1x + t2\y2 * t2\tv1z + t2\z2 * t2\tv1y)/s\scalex + s\shiftx)/IW
					t2\u3 = ((t2\x3 * t2\tv1x + t2\y3 * t2\tv1z + t2\z3 * t2\tv1y)/s\scalex + s\shiftx)/IW
					
					t2\v1 = ((t2\x1 * t2\tv2x + t2\y1 * t2\tv2z + t2\z1 * t2\tv2y)/s\scaley + s\shifty)/IH
					t2\v2 = ((t2\x2 * t2\tv2x + t2\y2 * t2\tv2z + t2\z2 * t2\tv2y)/s\scaley + s\shifty)/IH
					t2\v3 = ((t2\x3 * t2\tv2x + t2\y3 * t2\tv2z + t2\z3 * t2\tv2y)/s\scaley + s\shifty)/IH
...
Function PXWC_BuildMap()
	...
	;Cria Superficies
	TBlend = GetProperty$(Find_EntityByClassname%("worldspawn"), "blend"):If TBlend = 0 Then TBlend = 2 ;Blending Mode das texturas
	TextureRoot$ = GetProperty$(Find_EntityByClassname%("worldspawn"), "texture") ;Diretorio das texturas
	For s.surface = Each surface
		...
		If FLAT_TEXTURE = 0
			...
			tx = TextureWidth(x\hand)
			ty = TextureHeight(x\hand)
			
			;text1 = LoadTexture("Decals/shadow.bmp",4)
			x\brush = CreateBrush()
			BrushTexture x\brush,x\hand,0,1
			;BrushTexture x\brush,text1,0,2
